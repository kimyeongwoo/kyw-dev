...
test("adapter applies prepared predecessor continuity only for exact terminal delivery work", async () => {
  const genesis = checkpointFixture();
  const advanced = createStandardDeliveryContinuityCheckpoint({
    repository: "owner/repository",
    sourceMainSha: "a".repeat(40),
  ...
  let implementationDispatchCalls = 0;
  let implementationApplyCalls = 0;
  const implementation = await runTaskArtifactCommand(implementationArgs, {
    ...
    applyStandardDeliveryContinuityTransition: async () => {
      implementationApplyCalls += 1;
    },
  });
  assert.equal(implementationDispatchCalls, 1);
  assert.equal(implementationApplyCalls, 0);
  ...
  const selected = await runTaskArtifactCommand(deliveryArgs, {
    hydratePriorStandardDeliveries: async () => {
      deliveryEvents.push("hydrate");
      return hydrated();
    },
    resolveTaskDispatch: async () => {
      deliveryEvents.push("dispatch");
      return {
        outcome: "SELECTED",
        action: "DELIVER",
        task: {
    ...
    applyStandardDeliveryContinuityTransition: async (options) => {
      deliveryEvents.push("apply");
      assert.equal(options.selectedTaskId, "0004");
      assert.equal(options.preparedCheckpoint, advanced);
      assert.equal("transitionToken" in options, false);
      return { write: { applied: true } };
    },
  });
  assert.deepEqual(deliveryEvents, ["hydrate", "dispatch", "apply"]);
  const serialized = JSON.stringify(selected);
  assert.doesNotMatch(
    serialized,
    /continuityTransitionToken|preparedCheckpoint|apply-continuity/,
  );

  let noPreparationDispatchCalls = 0;
  let noPreparationApplyCalls = 0;
  const withoutPreparation = await runTaskArtifactCommand(deliveryArgs, {
    hydratePriorStandardDeliveries: async () => ({
      deliveryLedger: {},
      deliveryExpectations: {},
    ...
    applyStandardDeliveryContinuityTransition: async () => {
      noPreparationApplyCalls += 1;
    },
  });
  assert.equal(noPreparationDispatchCalls, 1);
  assert.equal(noPreparationApplyCalls, 0);
  assert.equal(withoutPreparation.action, "DELIVER");

  let terminalDispatchCalls = 0;
  let terminalApplyCalls = 0;
  const terminal = await runTaskArtifactCommand(deliveryArgs, {
    hydratePriorStandardDeliveries: hydrated,
    resolveTaskDispatch: async () => {
      terminalDispatchCalls += 1;
      return { outcome: "ALL_COMPLETE" };
    },
    applyStandardDeliveryContinuityTransition: async () => {
      terminalApplyCalls += 1;
    },
  });
  assert.equal(terminalDispatchCalls, 1);
  assert.equal(terminalApplyCalls, 0);
  assert.equal("continuityTransitionToken" in terminal, false);

  let mismatchedDispatchCalls = 0;
  let mismatchedApplyCalls = 0;
  await assert.rejects(
    runTaskArtifactCommand(deliveryArgs, {
      hydratePriorStandardDeliveries: hydrated,
      resolveTaskDispatch: async () => {
        mismatchedDispatchCalls += 1;
        return {
          outcome: "SELECTED",
          action: "DELIVER",
          task: {
      ...
      applyStandardDeliveryContinuityTransition: async () => {
        mismatchedApplyCalls += 1;
      },
    }),
    (error) => error?.code === "INVALID_DELIVERY_SELECTION",
  );
  assert.equal(mismatchedDispatchCalls, 1);
  assert.equal(mismatchedApplyCalls, 0);

  let nullDispatchCalls = 0;
  let nullApplyCalls = 0;
  await assert.rejects(
    runTaskArtifactCommand(deliveryArgs, {
      hydratePriorStandardDeliveries: async () => ({
        deliveryLedger: {},
      ...
      applyStandardDeliveryContinuityTransition: async (options) => {
        nullApplyCalls += 1;
        return applyStandardDeliveryContinuityTransition(options);
      },
    }),
    /continuity transition input|checkpoint must be an object/i,
  );
  assert.equal(nullDispatchCalls, 1);
  assert.equal(nullApplyCalls, 1);
});

test("adapter apply failure exposes no delivery selection or downstream mutation", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "kyw-adapter-apply-failure-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const tasksRoot = path.join(root, "docs", "tasks");
  await mkdir(tasksRoot, { recursive: true });
  const checkpointPath = path.join(
  ...
  const checkpointBefore = "preserve exact checkpoint bytes\n";
  await writeFile(checkpointPath, checkpointBefore, "utf8");
  const preparedCheckpoint = checkpointFixture().checkpoint;
  const events = [];
  ...
  try {
    selectedResult = await runTaskArtifactCommand(
      [
        "dispatch",
        "--tasks-root",
        tasksRoot,
      ...
      ],
      {
        ...
        applyStandardDeliveryContinuityTransition: async (options) => {
          ...
          throw new Error("injected atomic apply failure");
        },
      },
    );
  ...
  assert.match(observedError?.message ?? "", /injected atomic apply failure/);
  assert.equal(selectedResult, undefined);
  assert.deepEqual(events, ["hydrate", "dispatch", "apply"]);
