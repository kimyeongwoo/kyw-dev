...
The path planner treats executable code under `skills/` as runtime, instructions and behavior templates as behavior, and pure guidance separately. Unknown, added, removed, or renamed paths receive conservative coverage. CI keeps a stable required aggregate and validates that selected roles ran successfully and omitted roles were intentionally unnecessary. Supported OS/Node coverage remains available for runtime, filesystem, installation, and release changes.
