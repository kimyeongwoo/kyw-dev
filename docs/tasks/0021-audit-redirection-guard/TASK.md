...
- [ ] AC-01: `0021` is proven unused across Task directories and fresh local/remote refs; all initial staged, unstaged, and untracked changes are attributable; the dedicated branch starts at the exact Task 0020 candidate; and the reviewed Task 0020 evidence is preserved first in a documentation-only checkpoint without moving the old branch or SHA.
- [ ] AC-02: Output-to-file detection follows the supported host shell's quote/escape rules and blocks real `> file`, `>> file`, `1>file`, `2>file`, and `2>>file` operators without a command-specific whitelist or new dependency.
- [ ] AC-03: Quoted/escaped literal `>` text, including `node -e "const f = x => x"`, is not a mutation attempt, while a mixed command with a quoted literal followed by a real redirect is still blocked; supported single quote, double quote, escaped quote, and backslash cases are deterministic.
- [ ] AC-04: The `2>&1` policy is grounded in the current executor/shell path and an actual shell regression, explicitly preserved or blocked, and does not create an exemption for `2>file`, `2>>file`, or unproven descriptor syntax.
- [ ] AC-05: Each output-redirection reason records the exact operator and zero-based offset plus quote/escape state and bounded redacted local context, including a redirect beyond offset 600; diagnostics do not unnecessarily expose a whole command, credential, or normal user/temporary path.
- [ ] AC-06: Read-only mode still fails on every executable file redirect or other mutation attempt, harmless literals no longer cause `READONLY_MUTATION_ATTEMPT`, fix-mode plan/scope checks remain unchanged, and tree/status/auth/no-artifact/cleanup invariants remain enforced.
...
- [ ] Implement the smallest dependency-free shell-aware detector and match-local diagnostic changes.
- [ ] Add and run the requested deterministic regression matrix and supported-shell execution check.
...
- ARCHITECTURE: Expected update to describe shell-aware output-redirection classification and match-local diagnostic evidence in the development-only audit smoke boundary.
- README: Expected update because the documented audit-smoke failure output fields change from a bounded command preview to more precise operator/offset/context evidence.
...
- Implement and test the detector and diagnostic changes.
...
Begin with `scripts/audit-smoke.mjs` and `test/audit-smoke.test.mjs`. Do not touch implementation until this pair validates and the initial commit records it. Preserve Task 0020 refs and do not perform any release-readiness action.
