...
| T-07 | AC-07 — Current CI, delivery roles, package, and release commands remain compatible. | Compare `ci.yml` behavior and required names, run hosted-CI/planner/foundation/distribution regressions, and assert no dependency, lockfile, lifecycle, or package-allowlist drift. | Regression | PASS | Existing CI/package/version owners are byte-unchanged; planner, full Stable, release, distribution, foundation, and isolation checks pass with no dependency, lifecycle, lockfile, or allowlist drift. |
