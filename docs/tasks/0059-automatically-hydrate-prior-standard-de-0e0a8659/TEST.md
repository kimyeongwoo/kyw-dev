...
- `$env:KYW_LIVE_GITHUB_HYDRATION='1'; node --test --test-name-pattern='live repository and GitHub hydration' test/task-delivery-hydration.test.mjs` — intermediate implementation exits 1 are preserved below; the READY-queue run exited 0 in 342.8 seconds, the first post-merge supplemental run exited 1 only on a stale test count after production hydration returned, and the corrected terminal-queue run exited 0 in 178.3 seconds.
...
- PASS — `node --test test/task-delivery-hydration.test.mjs` now reports 15 deterministic PASS and one credentialed live test skipped by default; the separate live command reports 1 PASS with no dispatcher call.
...
- FAIL → PASS — after PR #46 and its first-attempt post-main run succeeded, the production hydrator correctly returned the terminal exact-invocation dependency closure Tasks 0057–0059 with all three evaluator-satisfied, but the supplemental live test still expected the READY-queue total 29 and exited 1. The assertion now distinguishes the 28-outcome READY transition from the three-outcome terminal closure, limits chronology assertions to collected Tasks, and the corrected fresh run exited 0 in 178.3 seconds.
