...
- Capability probe 2 later used one model call and one completed turn with zero retries. The model proposed the exact requested `git rev-parse --show-toplevel` command, and the tool router received its PowerShell 7 wrapper, but the sole action was declined before command execution with exit `-1` and `blocked by policy`; the outer process exited `0`. JSONL parsing, model-process and turn completion, action/final-message capture, sanitization, protected-state checks, and cleanup completed, but there was no completed command output or repository-root evidence. The retained verdict is `BLOCKED_EVIDENCE_INCOMPLETE`.
- A subsequent read-only policy-source investigation made zero model calls and zero `codex exec` calls; only `codex --version`, `codex --help`, and `codex exec --help` were inspected. It localized the rejection to the router policy-gate stage but could not identify a policy name/code, approval request or response, or shell-child PID. Approval, sandbox, classifier, and wrapper attribution therefore remain unproven, the exact policy source is `UNRESOLVED`, and no code change was indicated. Its verdict is `BLOCKED_POLICY_SOURCE_UNRESOLVED`.
