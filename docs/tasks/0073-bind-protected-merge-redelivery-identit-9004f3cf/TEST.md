...
- Actual file-symlink creation was unavailable on this Windows host; the unchanged branch remains covered by its existing regression and the forthcoming POSIX exact-head CI.
- Ordinary exact-head PR CI, protected merge, post-main CI, and final production-evaluator satisfaction remain the separate STANDARD delivery ledger and are not pre-claimed here.
