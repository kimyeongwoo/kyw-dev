...
Immediately before spawning the real publish command, the publish helper reads canonical `ci.yml` identity and exact main-push SHA through GitHub API, reconciles the latest run/attempt, and verifies the required aggregate and selected jobs. API errors, incomplete pages, ambiguity, and unsuccessful evidence fail closed. Workflow wiring and mock-publisher tests verify the actual boundary, not merely helper existence.
