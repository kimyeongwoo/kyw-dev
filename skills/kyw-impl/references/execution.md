...
At most one prior `STANDARD` outcome may remain uncovered; collect its full GitHub graph through an invocation-local command cache and the unchanged evaluator, never covered GitHub objects/logs. Empty history may prepare genesis. Missing, corrupt, stale, forked, mismatched, or over-gap existing continuity requires explicit migration/rebaseline: normal dispatch has no automatic whole-history fallback. The separately authorized Task `0070` recovery is the sole limited exception: on its exact portable invocation, freeze and enforce the owner-source/test/document allowlist, pass focused proof, hydrate exactly one evaluator-satisfied Task `0069` frontier from the valid Task `0068` checkpoint, and prepare without writing before the sole dispatcher call. Any failure stops without a second dispatch or workaround. Persist no raw logs, secrets, API responses, mutable graph, or growing receipts; manual delivery flags are test/compatibility seams only and never production recovery input.
