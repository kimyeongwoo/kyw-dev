...
The exact route enters `ACTIVE_ALIGNED`; inactive and post-terminal prompts do not warn, select, redirect, or deliver. Matching Skill, mode, baseline, selected Task, acceptance, scope, action, target, and attempt continues without duplicate confirmation.

Before a changed criterion mutates Git, GitHub, dispatcher state, Task/Test, permanent truth, or implementation, enter `CHANGE_PENDING`, issue a fresh old/new warning with concrete implementation, mutable Task/Test, permanent-owner, verification, and delivery impacts plus action, target, scope, attempt, and facts, then leave an empty mutation trace. A combined routed message activates once and cannot confirm its own change.

Only the trusted current user's immediate next applicable turn explicitly and unambiguously reconfirming the exact unchanged warning enters `RECONFIRMED_BOUNDED`. Synchronize only affected mutable owners before executing its exact bounds. The selected terminal pair is never mutable. Skill, mode, route, or Task replacement expires and requires its own exact route. Cancellation, decline, ambiguity, intervention, stale response, fact drift, or added bounds clears or replaces the warning. Safety, secrets, honest evidence, unrelated work, immutable pairs, exact routing, and separate external authorities remain non-waivable.
...
Every stage begins by revalidating the identities established above and every earlier stage. The fixed order is:

### 1. exact-path commit

Derive the selected Task's exact verified path set from its scope, final repository diff, Test evidence, documentation impact, and any valid predecessor-only checkpoint transition. Each included path must be required for the selected outcome; every required selected path must be included. Reject ambiguous ownership, missing/type/mode drift, unexpected generated bytes, or overlap that cannot be separated from user work.

Stage only the selected Task's exact verified path set with literal pathspecs. Preserve unrelated tracked, untracked, staged, and generated work exactly. Compare the staged tree, modes, and bytes with the proven set before committing, and ensure the commit contains exactly those intended changes. Never use broad staging, cleanup, reset, or destructive recovery. If pre-existing staging or path overlap prevents proof, block.

Create one ordinary selected-Task commit only if no matching completed commit already exists. Record its exact SHA and parents. Do not amend or manufacture a replacement on resume.

### 2. non-force push

Revalidate branch, local head, upstream, remote repository, and absence of drift. Before this first external mutation, prove that the committed selected head contains the hardened exact-SHA workflow contract and that GitHub reports its exact workflow ID, name, path, and active state. Push the exact selected branch normally once. Never force, force-with-lease, delete, rewrite, or fall back to another remote/credential. A completed remote head at the exact commit is observation, not permission to push again.
...
Observe, never trigger, the required PR actual-head workflows and jobs for the exact selected head. The current hardened contract is `HARDENED_EXACT_HEAD`; trusted expectations bind repository, workflow ID/name/path, base ref/SHA, outcome SHA, and behavioral, quality, and packed job-name sets.

Keep the run-level latest attempt distinct from each logical job's actual execution attempt. Reconcile bounded `filter=all`, `filter=latest`, and attempt-specific job collections. Each checkout-bearing job must provide valid `KYWCIEVIDENCE` with repository, role `PR_ACTUAL_HEAD`, exact checkout SHA, run/job/attempt identity, and expected parents. A projected earlier execution is reusable only when the API envelope, chronology, steps, and log uniquely prove it is the same untouched execution. Later actual execution supersedes; failure, cancellation, skipped/incomplete state, or invalid/missing evidence never falls back to an earlier success.

### 5. synthetic merge compatibility

Observe the separately named synthetic merge compatibility job after actual-head proof. It must assert role `PR_MERGE_COMPATIBILITY`, the exact synthetic checkout, and exactly two ordered expected base/head parents, then complete its required combined-state check. A job only at `refs/pull/<number>/merge` is merge compatibility, never actual head. Role confusion, reused identities, parent drift, or incomplete evidence blocks.
