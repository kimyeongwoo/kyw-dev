...
1. 이 문서 묶음의 내용을 로컬 `kyw_dev` 저장소 루트에 둔다.
...
`$kyw-audit`이 구현되기 전에는 아래 프롬프트를 사용한다.

```text
관련 없는 기능을 추가하지 말고 Task 000N을 독립 검수한다.

영구 문서, 해당 TASK.md와 TEST.md, 실제 구현, 관련 diff 또는 이력을 확인하고 다음을 검증한다.
1. 모든 완료 조건에 구현 근거가 있는가.
2. 의미 있는 신규 동작과 오류 경로마다 테스트 또는 명시적인 미검증 사유가 있는가.
3. 기록된 테스트 결과를 재현할 수 있는가.
4. SPEC, ARCHITECTURE, README, AGENTS가 영향받은 부분에서 동기화되어 있는가.
5. 범위 밖의 동작이 추가되지 않았는가.

현재 Task 범위에 명백히 속하는 문제는 수정하고 TASK.md와 TEST.md를 갱신한 뒤 관련 검증을 다시 실행한다. 범위 밖 문제는 새 Task 후보로 분리한다. 마지막에 근거와 함께 PASS 또는 BLOCKED로 판정한다.
```
