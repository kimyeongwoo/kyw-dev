...
The smoke publishes no repository result artifact. A successful run prints a compact summary containing the Codex version, requested model/effort, source-read proof, before/after fixture hashes and Git status, mutation/plan ordering, changed paths, test result, and final verdict. A mutation failure instead prints only a bounded redacted diagnostic naming the offending command or file-change kind, zero-based event index, detector reason, and tree/Git-status invariance. Credentials and absolute user/temporary paths are removed, and temporary repositories plus copied authentication are deleted after every run.
